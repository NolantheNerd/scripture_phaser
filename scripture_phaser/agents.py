...
class BaseAgent:
    ...
    def _validate(self, ref):
        start_ref, end_ref = self.split_reference(ref)
        if (
            not self.validate_reference(start_ref) or \
            not self.validate_reference(end_ref) or \
            not self.validate_reference_pair(start_ref, end_ref) \
        ):
